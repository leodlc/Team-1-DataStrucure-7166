...
int main() {
	...
	do{
		...
		switch (stoi(opc)) {
			...
			break;
			case 2:
				archivo.leerArchivoAlumno(listaAlumno,"Alumnos.txt");
				if (listaAlumno.estaVacia()) {
					cout << "Lista vacia primero ingrese datos." << endl;
				}
				else {
					cout << "-------LISTA DE ALUMNOS-------" << endl;
					listaAlumno.recorrer([&](Persona persona) {
						++cont;
						cout << "El usuario del Alumno-> " << cont << " es: " << persona.getUsuario() << endl;
						});
					cont = 0;
				}
			break;
			case 3:
			{
				...
				if ( listaProfesor.estaVacia( ) ) {
					...
					listaProfesor.recorrer( [ & ]( Persona persona ) {
						++cont;
						cout << "El usuario del Profesor-> " << cont << " es: " << persona.getNombre( ) << endl;
