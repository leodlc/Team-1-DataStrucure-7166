...
void Administrador::pantallaAdmin(void) {
	...
	do {
		...
		switch (stoi(opc)) {
		case 1:
			newAlumno = registrarAlumno();
			listaAlumno.revisarCorreoUsuario(&newAlumno);
			listaAlumno.insertar(newAlumno);
			if (!listaAlumno.estaVacia()) {
				archivo.escribirArchivoAlumno(listaAlumno, "Alumnos.txt");
			}
			break;
		case 2:
			newProfesor = registroProfesor();
			listaProfesor.revisarCorreoUsuario(&newProfesor);
			listaProfesor.insertar(newProfesor);
